...
oof_train = np.zeros((len(train),11))
...
for index, (train_index, val_index,val_orig_idx ) in enumerate(new_splits[0:], start=0):
    ...
    for it in range(1):
        ...
        with torch.no_grad():
            ...
            val_preds = np.vstack(pred_list)
                
        oof_train[val_orig_idx,:] = val_preds
...
np.save("oof_train/oof_train_seq2seq_lb940.npy",oof_train)
np.save("oof_test/oof_test_seq2seq_lb940.npy",test_preds_all/5)
